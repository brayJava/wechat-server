...
public class WechatWDController {
     ...
     * @return
     */
    @RequestMapping(value = "/c/{articleId}", produces = "text/html;charset=utf-8")
    @ResponseBody
    public String wd3(HttpServletRequest request, Model model, HttpServletResponse response, @PathVariable int articleId) {
        ...
        MobileUtil.analysisMobileFrom(request,redisObj,article.getWyArticle().getUserId());
        //1.从redis缓存中查询
        String showhtml = String.valueOf(redisObj.getRedisValueByKey("wd_3_article:" + articleId));
        if (!StringUtils.isEmpty(showhtml) && !"null".equals(showhtml)) {
            return showhtml;
        }
        //取缓存
        String html = String.valueOf(redisObj.getRedisValueByKey("wd_3_content:" + articleId));
        if (StringUtils.isEmpty(html) || "null".equals(html)) {
            model.addAttribute("article", article);
            log.info("返回连接：{}", article.getWyArticle().getGobackUrl());
            //手动渲染
            SpringWebContext ctx = new SpringWebContext(request, response,
                    request.getServletContext(), request.getLocale(), model.asMap(), applicationContext);
            html = thymeleafViewResolver.getTemplateEngine().process("html/wd-3/image_content", ctx);
            redisObj.saveDataToRedis("wd_3_content:" + articleId, html);
        }
        article.setContentHtml(html);
        model.addAttribute("article", article);
        //手动渲染原文
        SpringWebContext ctx = new SpringWebContext(request, response,
                request.getServletContext(), request.getLocale(), model.asMap(), applicationContext);
        showhtml = thymeleafViewResolver.getTemplateEngine().process("html/wd-3/index", ctx);
        redisObj.saveDataToRedis("wd_3_article:" + articleId, showhtml);
        log.info("日志输出：{}", request.getRequestURI().toString());
        return showhtml;
    }
